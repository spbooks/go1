...
func (image *Image) EditRoute() string {
	return "/image/" + image.ID + "/edit"
}
...
// A map of accepted mime types and the extension we associated them with
...
func (image *Image) CreateFromURL(imageURL string) error {
	...
	// The returned value from io.Copy is the size of bytes copied
	image.Size = size
